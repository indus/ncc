...
function NCC(options, cb) {
    ...
    if (options)
        for (var key in NCC.options)
            options[key] && (NCC.options[key] = options[key]);

    !NCC.options.spawn && (NCC.chromePid = -1)
    if (NCC.options.spawn && !NCC.chromePid) {
        ...
        for (var i = 0, l = args.length; i < l; i++)
            (reg = regExp.exec(args[i])) && (args[i] = args[i].replace(reg[0], NCC.options[reg[1].toLocaleLowerCase()]))
            ...
            for (var i = 0, l = list.length; i < l; i++) {
                ...
                if (list[i].title == "ncc" && list[i].webSocketDebuggerUrl) {
                    ...
                    if (NCC.options.retry--)
                        ...
                        cb && cb("[ncc rd] 'ncc' not found in inspectables")
                }
            }
        })
    }).on('error', function (e) {
    ...
    return canvas = canvas||NCC.createCanvas(true);
}
...
function defineProp(props, key, type, val) { //TODO: validate setter
    ...
    props['_' + key] = { value: val, writable: true, enumerable: debug }
    props[key] = {
    ...
    getContext: {
        value: function (contextId, cb) {
            if (contextId == "2d") {
                ...
                return this._ctx = Object.defineProperties(ctx, ctxProps);
            } else {
    ...
    _base64: {
        ...
        set: function (base64) {
            ...
            this._(function (err, val) {
                ...
                img.onload && img.onload(img)
            })

            return this._base64_ = base64;
        },
        enumerable: debug
    },
    _toFs: {
        value: function (path, cb) {
            ...
            var head = typeRegExp.exec(this._base64_), type = path.split('.').pop();

            if (!head || !head[1] || head[1] != type)
                cb && cb("type missmatch " + type + " <> " + head[1])
            ...
            if (dataRegExp.test(src)) {
                ...
                http.get(src, function (res) {
                    ...
                    if (res.statusCode != 200) {
                        img.onerror && img.onerror("loading image failed with status " + res.statusCode)
                        return;
                    }
